...
-- Point-in-time derived feature store. Append-only for distinct feature_keys;
-- derived features must never consume inputs whose knowledge_time exceeds the
-- feature's own knowledge_time.
...
-- Artist factor scores (momentum / relevance / audience fit / risk) with PIT
-- metadata. These are MODELED values and must never be presented as observed.
...
-- Expected-billing predictions (MODELED, PIT-stamped).
...
-- Relative-value calculations (MODELED, PIT-stamped).
...
-- Festival portfolio analytics (MODELED, PIT-stamped).
...
-- ---------------------------------------------------------------------------
-- metrics.artist_attention_observations - Wikimedia/Spotify attention signals
-- ---------------------------------------------------------------------------
-- Normalized source observations for cultural-velocity inputs. Reruns upsert
-- on observation_key (artist + source + period + metric_version).
...
-- Public dated tour observations for radius / exclusivity gap detection.
...
-- Primary/secondary ticket price observations for spread metrics.
...
-- Edition-level derived Festival-Bloomberg analytical metrics (versioned).
...
-- ===========================================================================
-- Drop any existing duplicate definition
...
CREATE TABLE IF NOT EXISTS core.secondary_ticket_observations (
    ...
    currency VARCHAR(3),
    fee_components_minor BIGINT,
    total_buyer_price_minor BIGINT,
...
CREATE TABLE IF NOT EXISTS metrics.ticket_price_spreads (
    id VARCHAR PRIMARY KEY,
    primary_tier_id VARCHAR NOT NULL,
    secondary_observation_id VARCHAR NOT NULL,
    absolute_spread_minor BIGINT,
    percentage_spread DECIMAL(20,8),
    buyer_margin DECIMAL(20,8),
    currency VARCHAR(3) NOT NULL,
    timestamp_delta_seconds BIGINT NOT NULL,
    quality_flags VARCHAR NOT NULL,
    arbitrage_candidate BOOLEAN NOT NULL,
