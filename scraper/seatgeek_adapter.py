...
class SeatGeekAdapter:
    ...
    def snapshot(self,raw:Mapping[str,Any],retrieved_at=None):
        ...
        return SeatGeekSnapshot(str(vals['event'] or ''),str(vals['listing'] or ''),str(vals['url'] or ''),str(vals['title'] or ''),p('ticket_type'),p('section'),p('row'),p('quantity'),p('price_minor','price'),p('currency'),p('fee_components_minor'),p('total_buyer_price_minor','total_price_minor'),True if active is None else bool(active),ts,digest,'seatgeek:'+str(vals['listing'] or 'unknown'),flags)
    def to_row(self,snapshot,edition_id):
        row=asdict(snapshot); row.update(edition_id=edition_id,source=self.source); row.pop('content_hash'); row.pop('provenance'); return row
