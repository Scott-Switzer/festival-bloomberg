...
CREATE TABLE festival_appearances (
    ...
    edition_key VARCHAR, event_name VARCHAR
);
...
def serving(tmp_path):
    ...
    # live history: 3 events before 2025-01-01, 9 total
    hist = []
    for i in range(9):
        hist.append((f"h{i}", "mbid::sheeran", f"Show {i}", f"2024-{i % 12 + 1:02d}-15", "Venue", "Chicago", "IL", "test"))
    # Barry: 4 pre-cutoff events so his PIT reconstruction is complete too.
    for i in range(4):
        hist.append((f"hm{i}", "mbid::manny", f"Manny Show {i}", f"2023-{i % 12 + 1:02d}-10", "Beacon", "New York", "NY", "test"))
    conn.executemany("INSERT INTO event_history VALUES (?, ?, ?, ?, ?, ?, ?, ?)", hist)
    conn.execute("INSERT INTO artist_markets (row_key, artist_key, market_key, observed_shows, first_play_date, last_play_date, future_events, ticket_evidence_count) VALUES "
                 "('m1','mbid::sheeran','chicago-il',3,'2023-05-01','2024-08-20',1,1),"
                 "('m2','mbid::sheeran','london-gb',2,'2023-05-01','2024-06-10',0,1),"
                 "('m3','mbid::sheeran','madison-wi',1,'2024-04-01','2024-04-01',0,0),"
                 "('t1','mbid::tayl','chicago-il',5,'2022-01-01','2024-09-01',0,2),"
                 "('t2','mbid::tayl','london-gb',4,'2022-01-01','2024-09-01',0,2),"
                 "('t3','mbid::tayl','nyc-ny',6,'2022-01-01','2024-09-01',1,2),"
                 "('b1','mbid::manny','chicago-il',7,'2021-01-01','2024-07-01',2,3)")
    # f1 and f2 share the same event_key => real shared-festival-bill evidence for comps.
    conn.execute("INSERT INTO festival_appearances (appearance_key, artist_key, event_key, festival_key, festival_name, edition_year, event_date, billing_tier) VALUES "
                 "('f1','mbid::sheeran','ev-fest-glasto-24','fest-glasto','Glastonbury','2024','2024-06-25','HEADLINE'),"
                 "('f2','mbid::tayl','ev-fest-glasto-24','fest-glasto','Glastonbury','2024','2024-06-26','HEADLINE'),"
                 "('f3','mbid::manny','ev-fest-other-23','fest-other','Other Fest','2023','2023-07-01','SUB')")
...
def test_pit_features_use_only_pre_cutoff_evidence(serving, workspace):
    ...
    assert pit["prior_live_events"] >= 3  # all fixture events are before 2025
    # A cutoff before any evidence yields PIT_PARTIAL, never fake zeros-as-known.
