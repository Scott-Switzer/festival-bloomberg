...
def test_classify_source_document_observed_day():
    ...
    assert rows[0]["source_publication_time"].startswith("2024-05-23T00:00:00")
...
def test_derive_metrics_math():
    ...
    run = build_acquisition_run_row(
        ...
        requests=500, successful_responses=400, new_claims=10, new_cutoffs=2,
        new_unique_events_improved=5, new_warm_start_events=1,
    ...
    # zero requests -> per-1000 metrics are None (never fabricated)
    empty = derive_metrics(build_acquisition_run_row(provider="x", pipeline="y", requests=0))
...
def test_provider_accounting_never_mixes_statuses(tmp_path):
    ...
    cdx_run = build_acquisition_run_row(
        provider="commoncrawl_cdx", pipeline="OUTCOME_HUNTER",
        requests=100, successful_responses=90,
        not_found=90, rate_limited=0, http_failed=0,
        detail="CDX only",
    )
    wiki_run = build_acquisition_run_row(
        provider="wikipedia_mediawiki_api", pipeline="OUTCOME_HUNTER",
        requests=100, successful_responses=10,
        not_found=10, rate_limited=85, http_failed=5,
